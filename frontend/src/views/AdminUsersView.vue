    ...
    <div class="grid gap-3">
      <article v-for="row in page?.items || []" :key="row.user.id" class="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div class="grid gap-2 md:grid-cols-[1fr_auto] xl:min-w-[28rem]">
            <form class="flex gap-2" @submit.prevent="reset(row.user.id)">
              <input v-model="passwords[row.user.id]" minlength="8" placeholder="新密码" required type="password" />
              <button class="btn-secondary whitespace-nowrap" type="submit">重置</button>
            </form>
